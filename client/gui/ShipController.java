...
public class ShipController extends AbstractControl implements ActionListener, AnalogListener {
    ...
    public ShipController(Geometry floor, Node rootNode, AppSettings settings, GameClient client) {
        this.client = client;
        this.floor = floor;
        S.inputManager.addMapping("mouseRightPressed",
                new Trigger[] { new MouseButtonTrigger(MouseInput.BUTTON_RIGHT) });
        S.inputManager.addMapping("mouseMoved", new Trigger[] { new MouseAxisTrigger(MouseInput.AXIS_X, true),
